...
const assets = [
  { key: 'logoUrl', label: 'Logo horizontal', fallback: '/ainaka-logo-horizontal.png' },
  { key: 'symbolUrl', label: 'Símbolo da marca', fallback: '/ainaka-symbol.png' },
  { key: 'faviconUrl', label: 'Favicon', fallback: '/ainaka-symbol.png' },
]
...
export default function BrandEditor({ setNotice }: { setNotice: (message: string) => void }) {
  ...
  async function upload(key: string, event: ChangeEvent<HTMLInputElement>) {
    ...
    try { const response = await fetch('/api/upload', { method: 'POST', credentials: 'include', body: form }); const data = await response.json(); if (!response.ok) throw new Error(data.error); await saveSetting(key, data.url); setNotice('Imagem atualizada com sucesso.') } catch (reason) { setNotice(reason instanceof Error ? reason.message : 'Erro no envio.') }
  }
